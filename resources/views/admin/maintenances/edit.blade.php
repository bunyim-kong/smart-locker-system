...
<div class="p-6 max-w-2xl mx-auto">
    <div class="flex items-center gap-2 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Edit Maintenance</h1>
    </div>
    ...
    <form action="{{ route('maintenances.update', $maintenance->id) }}" method="POST"
        ...
        <div>
            ...
            <select name="locker_id" required
                ...
                @foreach ($lockers as $locker)
                    <option value="{{ $locker->id }}" @selected(old('locker_id', $maintenance->locker_id) == $locker->id)>
        ...
        <div>
            ...
            <textarea name="issue_des" rows="4" maxlength="1000" required
                      ...
                      class="w-full px-3 py-2 border {{ $errors->has('issue_des') ? 'border-red-400' : 'border-gray-200' }} rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('issue_des', $maintenance->issue_des) }}</textarea>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="report_date"
                       value="{{ old('report_date', $maintenance->report_date ? \Carbon\Carbon::parse($maintenance->report_date)->format('Y-m-d') : '') }}"
                       required
            ...
            <div>
                ...
                <input type="date" name="resolve_date"
                       value="{{ old('resolve_date', $maintenance->resolve_date ? \Carbon\Carbon::parse($maintenance->resolve_date)->format('Y-m-d') : '') }}"
                       required
